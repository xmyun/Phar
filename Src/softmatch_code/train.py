...
from utils import net_builder, get_logger, count_parameters, over_write_args_from_file, TBLog, get_optimizer, \
    get_cosine_schedule_with_warmup
from torchssl.algorithms.argument import str2bool
from torchssl.algorithms import get_algorithm
from torchssl.datasets.ssl_dataset import SSL_Dataset, ImageNetLoader
from torchssl.datasets.data_utils import get_data_loader
from torchssl.algorithms import name2alg
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        if args.dist_backend == 'nccl':
            ...
            print(f"world size: {args.world_size}, rank: {args.rank}, local rank: {args.gpu}")
    ...
    if args.dataset != "imagenet":
        ...
        _eval_dset = SSL_Dataset(args, alg=args.algorithm, name=args.dataset, train=False,
                                num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    dataset_dict = {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset}
    ...
    if 'imagenet' in args.dataset.lower():
        ...
        _net_builder = net_builder(args.net,
                                   ...
                                   {'first_stride': 2 if 'stl' in args.dataset else 1,
                                    ...
                                    'use_embed': False,
                                    'is_remix': True if args.algorithm == 'remixmatch' else False},
                                   )
    ...
    loader_dict['train_lb'] = get_data_loader(dataset_dict['train_lb'],
                                              args.batch_size,
    ...
    loader_dict['train_ulb'] = get_data_loader(dataset_dict['train_ulb'],
    ...
    loader_dict['eval'] = get_data_loader(dataset_dict['eval'],
    ...
    if args.resume and os.path.exists(args.load_path):
        try:
            ...
            logger.info("logging not scueess")    
            pass
    else:
...
if __name__ == "__main__":
    ...
    '''  
    parser.add_argument('-alg', '--algorithm', type=str, default='softmatch', help='ssl algorithm')
    ...
    port = get_port() # args.dist_url.split(':')[-1]
    args.dist_url = "tcp://127.0.1.1"  + ':' + str(port)
