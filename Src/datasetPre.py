...
from dataAug import Preprocess4Normalization, Preprocess4Sample, Preprocess4Rotation, Preprocess4Noise, Preprocess4Permute, Preprocess4Sample_test, Preprocess4Mask
...
def load_Uci_users(data, label, Domain_index):  # User_list, 
    ...
    datas, labels = filter_domain_list(data, label, Domain_index)  # Extract data in dataset/domain 1;
...
# 读取 指定的 用户: User_list
def load_Across_users(User_list, data, label, Domain_index):
    datas, labels = filter_domain_list(data, label, Domain_index)  # Extract data in dataset/domain 1; 
    datas, labels = filter_dataset(datas, labels)
    datas, labels = filter_label_max(datas, labels, 4) # Cutoff four activity.  2 is very high; 
    dataset = Extractor_Users(datas, labels, User_list, user_label_index=1)
...
def load_dataset(args):
    # print(args)
    # set_seeds(args.seed)
    # print("种子的数值", args.seed)
    path_label = os.path.join("/label_" + "20_120" + ".npy") 
    path_data = os.path.join("/data_" + "20_120" + ".npy") 
    path_label = "../../phar_1101/Datasets/Merge/OurData" + path_label # UniHarData
    path_data = "../../phar_1101/Datasets/Merge/OurData" + path_data #  OurData
    # print(path_label)
    ...
    if (args.cd == "SigDom" ): # Inner single dataset.  
        ...
        # shape is: data_train, label_train, data_vali, label_vali, data_test, label_test
        data_train, label_train, data_vali, label_vali, _, _\
            = further_split_train(data, splitTrain_again, result_tvt_2) # how to test. 
        # 注意不同数据集的 搭配，特别是 训练模型的指标
    
    # seq_len=20 # 3 lines for data reshape. 
    # [data_test] = reshape_data([data_test], seq_len)  
    # [label_test] = reshape_label([label_test], data.shape[1], seq_len) 
    
    pipeline = [Preprocess4Normalization(args.feature_num), Preprocess4Sample(args.seq_len, temporal=0.4) # args.model_cfg.feature_num)
            , Preprocess4Rotation()] 
    pipeline_test = [Preprocess4Normalization(args.feature_num), Preprocess4Sample_test(args.seq_len, temporal=0.4) # args.model_cfg.feature_num)
            , Preprocess4Rotation()] # , Preprocess4Mask(args.mask_cfg, full_sequence=True)
    # pipeline_tta = [Preprocess4Normalization(args.model_cfg.feature_num),Preprocess4Sample(args.seq_len, temporal=0.4) # Preprocess4Normalization(args.feature_num)
    ...
    if args.model != 'deepsense':
        ...
        data_set_test = IMUDataset(data_test, label_test, pipeline=pipeline_test)
        ...
        # data_set_test = FFTDataset(data_test, label_test, pipeline=pipeline[:1])
        data_set_vali = FFTDataset(data_vali, label_vali, pipeline=pipeline[:2])
        data_set_test = IMUDataset(data_test, label_test, pipeline=pipeline_test)
