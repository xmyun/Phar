...
class BenchmarkDCNN(nn.Module):
    ...
    def __init__(self, cfg, input=None, output=None):
        ...
        self.lin1 = nn.Linear(120, input * cfg.flat_num)
        
        self.lin2 = nn.Linear(input * cfg.flat_num, output)
    ...
    def forward(self, input_seqs, training=False):
        ...
        h = h.view(h.size(0), -1) # comment by xm h.size(0), h.size(1), h.size(2) * h.size(3)
        h = self.lin1(h)
        h = F.relu(F.tanh(torch.sum(h, dim=1)))
        h = self.normalize(h[:, None, None]) # self.normalize(h[:, :, None, None])
        h = self.lin2(h[:, :]) # self.lin2(h[:, :, 0, 0])
        return h
...
class PGD1:
    def __init__(self, model, eps=0.04, alpha=1/255, steps=1):
        self.model = model
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
    ...
    def _project(self, x_adv, x_orig):
        return torch.clamp(x_adv, x_orig - self.eps, x_orig + self.eps)
...
class CoTTA_attack(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, arg=None):
        ...
        self.transform = [Preprocess4Normalization_t(arg.model_cfg.feature_num),Preprocess4Sample_t(arg.seq_len, temporal=0.4)
                          ,Preprocess4Rotation_t(), Preprocess4Noise_t(), Preprocess4Permute_t()]
        # self.transform = [ Preprocess4Noise_t()]
        
        
        # self.transform1_norm = transforms.Compose([Preprocess4Normalization_t(arg.model_cfg.feature_num)])
        ...
        self.model_state, self.optimizer_state, self.model_ema, self.model_anchor = \
            copy_model_and_optimizer(self.model, self.optimizer)
        self.attack = PGD1(self.model_ema, eps=0.01, alpha=1 / 255, steps=1) #  eps=0.03, alpha=2 / 255,  raw=self.model,
        self.mt = mt_alpha
        self.rst = rst_m
        self.ap = ap
    ...
    def forward_and_adapt(self, x, model, optimizer,x_ema,epoch):
        ...
        x_attack = self.attack.attack(x,outputs) # Reduce attack
        ...
        discrepancy_loss = discrepancy_loss * J_list[0][1]*10
        if inputs[1]<np.exp(2): # epoch * 0.1
            optimizer.zero_grad()
        ...
        return standard_ema, discrepancy_loss, inputs[1]  
...
class CNN_UCI(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(9216, 6)
