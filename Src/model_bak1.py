...
class BenchmarkDCNN(nn.Module):
    ...
    def __init__(self, cfg, input=None, output=None):
        ...
        self.lin1 = nn.Linear(156, 400) #   input * cfg.flat_num, 400 
...
class CoTTA(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, arg=None):
        ...
        self.model_state, self.optimizer_state, self.model_ema, self.model_anchor = \
            copy_model_and_optimizer(self.model, self.optimizer) # arg.n_class
        ...
        # self.transform = [Preprocess4Normalization_t(arg.input),Preprocess4Sample_t(arg.seq_len, temporal=0.4) # 4: arg.input
        #                   ,Preprocess4Rotation_t(), Preprocess4Noise_t(), Preprocess4Permute_t()] # arg.len_sw
    ...
    def forward_and_adapt(self, x, model, optimizer,x_ema):
        ...
        outputs = self.model(x)
        if isinstance(outputs, tuple): # Which is remove at the latest version. 
            outputs, _ = outputs
        # Teacher Prediction
        anchor_prob = torch.nn.functional.softmax(self.model_anchor(x), dim=1).max(1)[0]
        standard_ema = self.model_ema(x)
        ...
        for i in range(N):
            ...
            outputs_  = self.model_ema(x).detach()   # 1,3,5
            outputs1_  = self.model_ema(self.transform1_norm(x)).detach() # x
            outputs_emas.append(outputs1_*0.7)
            # self.model_ema.cpu()
            # outputs2_  = self.model_ema(self.transform2_sample(x_ema)).detach()
            # outputs_emas.append(outputs2_*0.1) 
            outputs3_  = self.model_ema(self.transform3_rot(x)).detach()
            outputs_emas.append(outputs3_*0.8) 
            # outputs4_  = self.model_ema(self.transform4_noise(x.clone())).detach() 
            # outputs_emas.append(outputs4_*0.5) 
            outputs5_  = self.model_ema(self.transform5_permu(x)).detach() 
            outputs_emas.append(outputs5_*0.4) 
...
# Function for feature distribution measurements. 
...
def kmmd_dist(x1, x2):
    ...
    return diff.to(torch.device('cpu')).detach().numpy()
...
def threshold_determine(clean_feature_target, ood_detection):
    ...
    percentile_95 = test_deviations_sort[int(len(test_deviations_sort)*0.95)][0] # There is the postion about percent. 
...
def J_t(model, source, target):
    ...
    clean_feature_group = feature_test[np.where(ood_label_95==0)] # Test for 95? 99? 
...
def find_two_smallest(nums):
    ...
    # Remove the first smallest number from the list
    nums.remove(min1)

    # Find the second smallest number
    min2 = min(nums)
    index2 = nums.index(min2)

    # Add back the position of the first smallest number
    if index2 >= index1:
        index2 += 1
    return (min1, index1), (min2, index2)

# New version for the CoTTA
class CoTTA_attack(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, arg=None):
        ...
        self.attack = PGD(self.model, eps=0.08, alpha=1 / 255, steps=10) # eps=0.03, alpha=2 / 255,
        self.model_state, self.optimizer_state, self.model_ema, self.model_anchor = \
            copy_model_and_optimizer(self.model, self.optimizer)
        self.mt = mt_alpha
        self.rst = rst_m
    ...
    def forward_and_adapt(self, x, model, optimizer,x_ema,epoch):
        ...
        for i in range(N):
            inputs_dis=torch.tensor(0.0)
            distance_dis = torch.tensor(200.0)
            for i in np.arange(0, len(self.transform)+1, 1):
                ...
                if J_t_median < distance_dis:
                    distance_dis = J_t_median
                    outputs_1 = self.model_ema(input).detach()
                    outputs_emas.append(outputs_1)
                    # inputs_dis.append(distance_dis) 
                    inputs_dis += distance_dis
                # inputs_dis.append([J_t_median, input]) 
                
                

            # score_aug_val = [score_aug[0] for score_aug in inputs_dis]
            # score_inputs = [score_inputs[1] for score_inputs in inputs_dis]
            # distance_dis = min(score_aug_val) 
            # min_index = score_aug_val.index(min(score_aug_val))
            # corresponding_value = score_inputs[min_index]
            # print(corresponding_value)
            
            # (Sml1_val, Sml1_pos),(Sml2_val, Sml2_pos) =find_two_smallest(score_aug_val)
            # distance_dis = distance_dis + Sml1_val + Sml2_val
            # outputs_1 = self.model_ema(corresponding_value).detach()
            # outputs_emas.append(outputs_1)
            # outputs_2 = self.model_ema(score_inputs[Sml2_pos]).detach()
            # outputs_emas.append(outputs_2)
        ...
        loss = (softmax_entropy(outputs, outputs_ema)).mean(0) + inputs_dis.mean(0) # mean var[little work]
        print("Loss: ",(softmax_entropy(outputs, outputs_ema)).mean(0), "distance", inputs_dis)
        loss.backward()
...
def fetch_classifier(args):
    if 'lstm' in args.model:
        ...
        model = ClassifierGRU(args, input=args.input, output=4) # args.output
